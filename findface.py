...
while True:
    ...
    if img_file:
        for i, img in enumerate(img_file):
             print(img)
             biden_image = face_recognition.load_image_file(img)

             if face_recognition.face_encodings(biden_image):
                 biden_face_encoding = face_recognition.face_encodings(biden_image)[0]

                 known_face_encodings.append(biden_face_encoding)
                 known_face_names.append(img.split('/')[1].split('.')[0])
    ...
    for (top, right, bottom, left), name in zip(face_locations, face_names):
        ...
        cv2.imwrite(f"images/{name}.jpg", crop_img)
